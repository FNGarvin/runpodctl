...
run_step() {
    ...
    while [ $attempt -le $max_attempts ]; do
        ...
        if [ $attempt -lt $max_attempts ]; then
            echo "  [RETRY] $name failed (Attempt $attempt/$max_attempts). Retrying in 5s..."
            sleep 5
            attempt=$((attempt + 1))
        else
...
if [[ -z "$RUNPOD_API_KEY" ]]; then
    ...
    create_out=$(mktemp)
    if run_step "Pod Create" "$RUNPODCTL pod create --template-id bwf8egptou --compute-type CPU --name $POD_NAME > $create_out" "" 3; then
        ...
        if [[ -n "$POD_ID" && "$POD_ID" != "null" ]]; then
             ...
             sleep 5
             run_step "Pod List" "$RUNPODCTL pod list --output json | jq -e \"map(select(.id == \\\"$POD_ID\\\")) | length > 0\"" "" 3
    ...
    ep_out=$(mktemp)
    if run_step "Serverless Create" "$RUNPODCTL serverless create --template-id wvrr20un0l --compute-type CPU --gpu-count 0 --workers-max 1 --name '$EP_NAME' > $ep_out" "" 3; then
        ...
        if [[ -n "$EP_ID" && "$EP_ID" != "null" ]]; then
            ...
            if [ "$ready" = false ]; then
                ...
                # Don't return 1 here, let the next steps try to fail naturally or cleanup
            fi

            run_step "Serverless Get (Final Check)" "$RUNPODCTL serverless get $EP_ID" "" 3
            run_step "Serverless List" "$RUNPODCTL serverless list --output json | jq -e \"map(select(.id == \\\"$EP_ID\\\")) | length > 0\"" "" 5
            run_step "Serverless Update" "$RUNPODCTL serverless update $EP_ID --name \"${EP_NAME}-updated\"" "" 3
            
            echo "Testing Serverless Job Submission..."
            JOB_OUT=$(mktemp)
            # Submission is usually fast if the endpoint 'Get' works
            if run_step "Serverless Send (Job Submit)" "curl -s -X POST \"https://api.runpod.ai/v2/$EP_ID/run\" -H \"Content-Type: application/json\" -H \"Authorization: Bearer $RUNPOD_API_KEY\" -d '{\"input\": {\"test\": \"data\"}}' > $JOB_OUT" "" 3; then
            ...
            echo "Cleaning up Endpoint $EP_ID..."
            # Retry delete as well, just in case
            retry_count=0
            while [ $retry_count -lt 3 ]; do
                if $RUNPODCTL serverless delete $EP_ID; then break; fi
                sleep 5
